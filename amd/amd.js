...
(function(document) {
    ...
    *resolves，解析信息容器，形式为：resolves[name] = value，name为模块名，value为解析后的值（或者说是js代码执行后的结果）
    *config，配置信息容器，形式为：config[name] = url;name为模块名，url为模块对应的加载地址
    */
    var listeners = {}, resolves = {}, config = {};

    /*加载模块*/
    function addLoadListener(name, listener) {
        //name模块已被解析，其值为(模块已加载，且返回值为)resolves[name]，则立即执行require(name, fn)中的fn
        if(name in resolves) listener(name, resolves[name]);
        //模块未被解析且曾被请求过，则将fn存储在listeners模块请求容器中
        else if(listeners[name]) listeners[name].push(listener);
        else {
            //模块未被解析且未曾请求过，则将fn存储并该文件引入到网页中
            listeners[name] = [listener];
            if(config[name]) {
                var element = document.createElement('SCRIPT');
                element.src = config[name];
    ...
    function resolve(name, value) {
        //将解析后的值存储到resolves对象中，即，在name模块加载之后，将返回值value存储在resolves对象中
        resolves[name] = value;
